...
async function animateTreasureHunt() {
    try {
        ...
        if (playerChoice === 1) {
            ...
            currentSceneImg.src = 'fire.jpg'; // 显示对决守卫图片
...
async function handleDigChoice(location) {
    try {
        ...
        console.error("对决守卫选择操作失败：", error);
        // 显示敲开地面失败的提示信息并切换到相应失败场景图片
        messageContainer.innerHTML = '';
        showMessage("对决守卫守卫，被守卫击败！");
        // 设置失败标记为true
        hasFailed = true;
        // 切换到相应的失败场景图片
        currentSceneImg.src = 'temple_guard.jpg';
    ...
    return new Promise((resolve) => {
        ...
        document.addEventListener('click', function (event) {
            if (event.target.textContent === "在神庙内巡视") {
                resolve(1);
            } else if (event.target.textContent === "对决守卫") {
                resolve(2);
            }
        });
    });
